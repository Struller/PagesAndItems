...
abstract class ExtensionPiPluginHelper //extends ExtensionsHelper
{
	...
	public static function importExtension($folder=null, $extensionnames = null, $autocreate = true, $dispatcher = null, $loadLanguage = false)
	{
		...
		if(!$isLoaded)
		{
			...
			if($extensions)
			{
				...
				return isset($loaded[$folder]) ? $loaded[$folder] : null;
			}
			return $results;
		}
		if($folder == '' || is_null($folder))
	...
	protected static function _load() //,$folder = '', $db = null)
	{
		...
		if (isset($user))
		{
			...
			$query .= ' AND type = '.$db->Quote('piplugin');
			$query .= ' AND state >= 0';
		...
		$db->setQuery($query);
		$db->query();
		if (!$piplugins = $db->loadObjectList())
		{
			if($db->getErrorMsg() != '')
			{
				//JError::raiseWarning('SOME_ERROR_CODE', 'Error loading Extensions: type: '.$type.', folder: '.$folder.', dberror: '. $db->getErrorMsg());
				JError::raiseWarning('SOME_ERROR_CODE', 'Error loading Extensions: type: piplugin, dberror: '. $db->getErrorMsg());
			}
			else
			{
				...
				*/
				//JError::raiseWarning('SOME_ERROR_CODE', 'Error loading Extensions: type: html,  Extension seem not installed');
			}
			return false;
		}
		return $piplugins;
