...
/**
 * ExtensionPagetype  helper class
 *
 */
...
abstract class ExtensionPagetypeHelper //extends ExtensionsHelper
{
	...
	protected static function _load() //,$folder = '', $db = null)
	{
		...
		if (isset($user))
		{
			...
			$query .= ' AND type = '.$db->Quote('pagetype');
			$query .= ' AND state >= 0';
		...
		$db->setQuery($query);
		$db->query();
		if (!$pagetypes = $db->loadObjectList())
		{
			if($db->getErrorMsg() != '')
			{
				//JError::raiseWarning('SOME_ERROR_CODE', 'Error loading Extensions: type: '.$type.', folder: '.$folder.', dberror: '. $db->getErrorMsg());
				JError::raiseWarning('SOME_ERROR_CODE', 'Error loading Extensions: type: pagetype, dberror: '. $db->getErrorMsg());
			}
			else
			{
				...
				*/
				//JError::raiseWarning('SOME_ERROR_CODE', 'Error loading Extensions: type: pagetype,  Extension seem not installed');
			}
			return false;
		}
		return $pagetypes;
