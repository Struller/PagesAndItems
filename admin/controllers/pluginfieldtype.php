...
*/
class PagesAndItemsControllerPluginFieldtype extends PagesAndItemsController
{
	...
	function __construct( $config = array())
	{
		...
		*/
		...
		if($this->controller->joomlaVersion < '1.6')
		{
			//J15
			require_once(JPATH_COMPONENT_ADMINISTRATOR.DS.'controllers'.DS.'controllerJ15.php');
		}
		...
		if($cmd)
		{
			...
			if (strpos($cmd, '.') != false)
			{
				...
				else
				{
					...
					$newConfig = array('base_path' => $this->pathPluginsFieldtypes.DS.$plugin.DS);
					//if($version->getShortVersion() < '1.6')
					//{
					//	$controller = controllerJ15::getInstance('PagesAndItems')); 
					//}
		...
		*/
		...
		if(file_exists($this->pathPluginsFieldtypes.'/'.$plugin.'/controllers/'.$plugin.'.php'))
		{
			require_once($this->pathPluginsFieldtypes.'/'.$plugin.'/controllers/'.$plugin.'.php');
			$className = 'PagesAndItemsController'.$plugin;
			$this->pluginController = new $className(); //PagesAndItemsControllerPI_Fish();
		}
