...
if($this->helper->app->isAdmin()){
	//only show footer in backend

	$document =& JFactory::getDocument();
	$document->addStylesheet('components/com_pagesanditems/css/pagesanditems.css'); 
	...
	echo '<tr>';
	echo '<td class="text_right">';
	echo $this->helper->pi_strtolower(JText::_('JVERSION'));
	echo '</td>';
	echo '<td class="five_pix">';
	echo '=';
	echo '</td>';
	...
	if($this->pi_config['version_checker']){
		...
		echo '</td>';
		echo '<td>';		
		$latest_version_message = $this->helper->app->getUserState( "com_pagesanditems.latest_version_message", '');
