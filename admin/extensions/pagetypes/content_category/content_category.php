...
//
class PagesAndItemsExtensionPagetypeContentCategory extends PagesAndItemsExtensionPagetype
{
	function onGetPageTypeIcons($icons,$pageType,$dirIcons, $component)
	{
		if($pageType != 'content_category')
		{
			return false;
		}
	...
	function onGetPageItems(&$html,$model)
	{
		$html = '';
		/*
		add here extra for category items like page items
		*/
		//TODO from an own class underlayingCategories
		require_once(JPATH_COMPONENT_ADMINISTRATOR.DS.'includes'.DS.'lists'.DS.'categorieslist.php');
		$CategoriesList = new CategoriesList();
		$html .= $CategoriesList->getCategories($html,$model,'maxLevel',true,true,true,'com_content'); //($html,$model,'maxLevel',false) if we want the slider is not open
	...
	function onAfterSave($menu_id, $data, $isnew,$item)
	{
		...
		if(JRequest::getVar('create_new_category', ''))
		{
			...
			if($itemCategory = $modelCategory->getItem()) //$controller->item_save($data))
			{
				...
				$database->setQuery( "UPDATE #__menu SET link='index.php?option=com_content&view=category&id=$itemCategory->id' WHERE id='$menu_id' ");
				$database->query();
				return true;
			}
			return true;
	...
	function onGetLists(&$lists,$pageMenuItem,$view)
	{
		...
		$addtop .= '<td colspan="2">';
		$addtop .= 'display from pagetypes/content_category/content_category.php lists->add->top';
		$addtop .= '<br />';
		$addtop .= 'the green color in id from pagetypes/content_category/content_category.php lists->display-id ';
		$addtop .= '<br />';
		...
		$addbottom = '<tr>';
		$addbottom .= '<td>';
		$addbottom .= 'display from pagetypes/content_category/content_category.php lists->add->bottom';
		$addbottom .= '</td>';
		$addbottom .= '</tr>';
